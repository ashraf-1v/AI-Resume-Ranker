...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.subheader("4. Admin Analytics")
        if st.button("Generate Analytics Report"):
            with st.spinner("Training models & plotting results..."):
                # 1. Trigger the backend to train models and create the plot
                response = api_request("post", "/api/generate-visualizations")
                ...
                if "error" in response:
                    ...
                    st.info(
                        "The performance plot has been saved to your project folder as 'analytics_visualizations.png'.")

                    # 2. OPTIONAL: Display the numerical results immediately
                    report = api_request("get", "/api/performance-report")
                    if "report" in report and "model_performance" in report['report']:
                        metrics = report['report']['model_performance']['performance']
                        st.write("---")
...
def render_candidate_card(candidate, index):
    ...
    with st.container():
        ...
        with col3:
            ...
            # Hired status checkbox - defaults to False (not hired)
            current_status = candidate.get('hired', False)
            is_hired = st.checkbox(
                "✅ Hired" if current_status else "❌ Not Hired", 
                value=current_status, 
                key=f"hired_{candidate['id']}"
            )
            
            # Update hiring status if checkbox changed
            if is_hired != current_status:
                api_request("post", "/api/update-hiring-status",
                            json={"candidate_id": candidate['id'], "hired": is_hired})
                candidate['hired'] = is_hired
                st.rerun()  # Refresh to update the label
